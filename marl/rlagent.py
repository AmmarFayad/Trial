...
class Neo(object):
  ...
  def update_rollout(self, obs, reward, mask):
    self.value, self.action, self.action_log_prob, self.states = self.actor_critic.act(self.rollouts.obs[-1],
              self.rollouts.recurrent_hidden_states[-1],self.rollouts.masks[-1],deterministic=False)
